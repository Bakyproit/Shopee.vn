...
export default function Cart() {
  ...
  const choosenPurchaseIdFromLocaltion = (location.state as { purchaseId: string } | null)
    ?.purchaseId
    ...
    setExtendedPurChases((prev) => {
      ...
      return (
        purchasesInCart?.map((purchases) => {
          const isChoosenPurchaseFromLocaltion = choosenPurchaseIdFromLocaltion === purchases._id
          return {
            ...purchases,
            disabled: false,
            checked:
              isChoosenPurchaseFromLocaltion ||
              Boolean(extendedPurchasesObject[purchases._id]?.checked)
          }
        }) || []
      )
    })
      ...
      <div className='container'>
        {extendedPurchases.length > 0 ? (
          ...
          <div className='text-center'>
            ...
            />
            <div className='font-bold text-gray-500 mt-5'>Giở hàng của bạn còn trống</div>
            <div className='text-center mt-5'>
              <Link
                to={path.home}
                className='bg-oranges hover:bg-oranges/80 transition-all px-10 py-2 rounded-sm uppercase text-white'
              >
                Mua ngay
              </Link>
            </div>
